        ...
        if (x<640):
            ...
            if (self.current_track["type"] == "video" and x>1820 and y<100):
                if (self.mpv_muted):
                    ...
                    self.mpv_player.keypress("m")
                    self.mpv_muted = False
                else:
...
#             
    def play_video(self):
        ...
        #make the Tkinter window appear above mpv's
        ...
        if (self.current_track['omx-subtitles'] != ''):
            self.mpv_player.loadfile(self.complete_path(self.current_track['location']), sub_file=self.complete_path(self.current_track['omx-subtitles']))
        else:
            self.mpv_player.loadfile(self.complete_path(self.current_track['location']))
