...
def load_extra_text(dirpath):
    ...
    if os.path.exists(dirpath+"/extra_text.txt"):
        efile = open(dirpath+"/extra_text.txt", "r", encoding='utf-8')
        for fname in efile:
            fname = fname.strip()
...
def process_video(vidfile):
    ...
    stlist = textwrap.wrap(subtext, width=53)
    subtext = "\n".join(stlist)
    ...
    sf = open(subtitles_file,"w",encoding="utf-8")
    ...
    record["omx-subtitles"] = "+/" + subtitles_file
    record["omx-subtitles-numlines"] = str(len(stlist))
    trackdict["tracks"].append(record)
...
if (not os.path.exists(sys.argv[1])):
    ...
    with open(sys.argv[1], 'r', encoding="utf-8") as jfile:
        data = jfile.read()
...
json_object = json.dumps(trackdict, indent=2, ensure_ascii=False)
